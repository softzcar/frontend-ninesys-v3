<template>
  <div>
    <b-table responsive small striped hover :items="dataTable.items" :fields="dataTable.fields">
      ...
      <template #cell(empleado)="data">
        {{ data.item }}
        <diseno-reasignacionSelect :options="optionSelect" :empleado="data.item.username" :idorden="data.item._id" :idempelado="data.item.empleado"></diseno-reasignacionSelect>
      </template>
    </b-table>
    <pre>
...
export default {
	...
	methods: {
		...
		/* 	async getDisenos() {
			await fetch(`${this.$config.API}/disenos/asignados`)
				.then(res => res.json())
				.then(res => {
					this.disenos = res;
					this.loading = false;
	...
	mounted() {
		this.$store.dispatch('disenos/getDisenos').then(() => {
			let tmpOptions = this.$store.state.disenos.empleados.filter(el => el.departamento === 'Diseño' || el.departamento === 'Jefe de diseño').map(el => {
				return {
					value: el._id,
					text: el.username,
				}
