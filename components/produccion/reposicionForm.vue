...
export default {
  ...
  methods: {
    ...
    async getEmpleados() {
      await axios.get(`${this.$config.API}/empleados`).then((res) => {
    ...
    async createReposicion() {
      ...
      await axios
        ...
        .catch((err) => {
          this.$fire({
            title: 'Error en el retorno',
            html: `<p>Ocurrió un error el servidor dijo:</p><p>${err}</p>`,
            type: 'danger',
          })
        })
    },
