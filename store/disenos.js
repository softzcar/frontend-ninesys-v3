...
export const actions = {
  async getDisenos({ commit }) {
    await axios.get(`${this.$config.API}/disenos`).then((res) => {
      commit('setDisenos', res.data.disenos)
      commit('setEmpleados', res.data.empleados)
    })
  },
  async getDisenosAsignados({ commit }) {
    await axios.get(`${this.$config.API}/disenos/asignados`).then((res) => {
  ...
  async getDisenosTerminados({ commit }) {
    await axios.get(`${this.$config.API}/disenos/terminados`).then((res) => {
